					...
					for (unsigned int side = 0; side < Cube::SIDE_COUNT; ++side) {
						...
						if (bake_occlusion) {
							...
							for (unsigned int j = 0; j < 4; ++j) {
								const unsigned int edge = (int)Cube::g_side_edges[side][j];
								const int edge_neighbor_id = type_buffer[voxel_index + edge_neighbor_lut[edge]];
								if (contributes_to_ao(library, edge_neighbor_id)) {
									++shaded_corner[(int)Cube::g_edge_corners[edge][0]];
									++shaded_corner[(int)Cube::g_edge_corners[edge][1]];
								}
							}
							for (unsigned int j = 0; j < 4; ++j) {
								const unsigned int corner = (int)Cube::g_side_corners[side][j];
